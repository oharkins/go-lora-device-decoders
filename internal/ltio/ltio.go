// Package ltio decodes the shared Dragino LT22222-L / LT33222-L I/O payload layout.
package ltio
...
// Data is the shared decoded LT22222-L / LT33222-L uplink.
type Data struct {
...
// Decode parses a shared LT22222-L / LT33222-L payload. name is used in error messages.
func Decode(name string, u decoders.Uplink) (*Data, error) {
	b := u.Payload
	if len(b) != 11 {
		return nil, fmt.Errorf("%s: want 11 bytes, got %d", name, len(b))
	}
